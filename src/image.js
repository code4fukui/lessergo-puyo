...
export class GameImage {
    ...
    static initialize() {
       ...
       // ゲームタイトル画像を準備する
        GameImage.logoImage = document.getElementById("logo");
        GameImage.logoImage.width = Config.puyoImageWidth;
        GameImage.logoImaheight = Config.puyoImageHeight;
        GameImage.logoImage.style.position = 'absolute';
     GameImage.logoImage.style.left = 0 + 'px';
        GameImage.logoImage.style.top = 0 + 'px';
    ...
    static updateBatankyu(frame) {
        ...
        const y = -Math.cos(ratio * Math.PI * 2) * height / 3 + height / 3;
